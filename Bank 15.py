...
def process_pdf(file):
    ...
    # Clean headers to handle multi-line names found in some Axis PDFs
    ...
    for _, row in df.iterrows():
        ...
        # --- NEW AXIS SPECIFIC LOGIC ---
        # Checks for the 'AMOUNT(INR)' and 'DEBIT/CREDIT' indicator structure 
        if 'AMOUNT(INR)' in df.columns and 'DEBIT/CREDIT' in df.columns:
            v = clean_val(row['AMOUNT(INR)'])
            indicator = str(row['DEBIT/CREDIT']).upper().strip()
            if 'CR' in indicator: nature, amount = "Receipt", v
            elif 'DR' in indicator: nature, amount = "Payment", v
        
        # --- EXISTING BANK SPECIFIC RULES (FALLBACK) ---
        else:
